...
//void eeprom_parser(char* eeprom_buffer, char* name, char* i_type, char* channel, char* value, int name_size);
//void eeprom_set_value(char* name, char* i_type, char* channel, char* value);
...
int main(void){
  ...
  //char eeprom_buf[EEPROM_MAX_SIZE];
  char buf[SIZE];
  char name[SIZE];
  char disp[SIZE];
  char channel[SIZE];
  ...
  uint8_t res;
  char result;
  //result = usart_getchar();
  //serial_init();

  //res = usart_getString((uint8_t*)buf);
  ...
  strcpy(name, buf);
  //printString("nome dispositivo\n");
  //printString(name);
  usart_pstr("nome dispositivo:\n");
  usart_pstr(name);
  ...
  while(1){
    ...
    //printString(disp);
    //printString(name);
    //printString("parsing fatto\n");
    usart_pstr("parsing fatto\n");
    ...
    if(strcmp(disp,name) == 0){
      //printString("entrato disp corretto\n");
      switch(t){
        case DIGITAL_OUT:
        //printString("entrato nello switch\n");
          if(bit < 4){
            //printString(value);
            if(val > 0){
              PORTB |= (1<<bit);
            ...
            //printString("entrato nello switch else\n");
            OCR1CL = val;
          }
        break;

        case DIGITAL_IN:
        //printString("entrato nello digital_in\n");
          i_mask = (1<<bit);
...
/*
void eeprom_parser(char *eeprom_buffer, char* name, char* i_type, char* channel, char* value, int name_size){
  int idx_start = name_size+1;
  int buffer_size = strlen(eeprom_buffer);
  for(;idx_start<buffer_size; idx_start++){
    char src[256];
    while(*eeprom_buffer != ';'){
      *src = *eeprom_buffer;
      src++;
    ...
    parse_string(src, name, i_type, channel, value);
    eeprom_set_value(name, i_type, channel, value);
  }
}
